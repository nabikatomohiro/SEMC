...
#include "../../core/base_semc_noex.hpp"

#define PI 3.14159265358979323846

class SpectralDeconvolutionSEMC : public BaseSEMCNoEx {
    ...
    public:
        SpectralDeconvolutionSEMC(ModelOptions op, vector<vector<PriorParameter> > prior, GaussData data): BaseSEMCNoEx(op, prior), BaseSampling(op, prior), data_(data){};
...
ModelOptions op;
    ...
    for(int j=0;j<M_pops.size();j++){
        ...
        for(int i=0; i<times.size(); i++){
            // if(burn_in_array[0]*times[i] < M_pops[j]){
            //     continue;
            // }
            op.M_pop = burn_in_array[0]*times[i]/M_pops[j];
            op.sample_num = burn_in_array[0]*2*times[i];
            op.burn_in = burn_in_array[0]*times[i];
            string folder_name_base = "../output/SEMC_Noex/burn_in_" + to_string(burn_in_array[0]*times[i]) + "_M_pop_" + to_string(M_pops[j]) + "/";
